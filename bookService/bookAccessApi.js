...
function getBooksMatched(parameter,pageNumber){
    ...
    var promise = new Promise(function(reject,resolve){
        ...
        request('https://www.googleapis.com/books/v1/volumes?q='+parameter+'&startIndex='+pageNumber*MAX_ITEMS+'&orderBy=relevance'+allFilters, { json: true }, (error, response, body) => {
            ...
            //console.log("############");
            //console.log(items);
            var isAvailableFreePDF = false;
            var isAvailableFreeEpub = false;
            for (var i = 0; i < items.length; i++) {
                ...
                isAvailableFreePDF = (items[i].accessInfo.pdf.downloadLink === undefined) ? "" : items[i].accessInfo.pdf.downloadLink;
                isAvailableFreeEpub = (items[i].accessInfo.epub.downloadLink === undefined) ? "" : items[i].accessInfo.epub.downloadLink;
                dtoBookHeader.isAvailableFreePDF = Boolean(isAvailableFreePDF);
                dtoBookHeader.isAvailableFreeEpub = Boolean(isAvailableFreeEpub);
